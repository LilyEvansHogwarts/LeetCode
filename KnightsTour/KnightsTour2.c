...
void changeDegree(vector<vector<int>>& visited, vector<vector<int>>& degree, int x, int y, int flag) {
...
void dfs(vector<vector<int>>& visited, vector<vector<int>>& degree, int x, int y, int num, int& solutions, vector<vector<vector<int>>>& res) {
    num++;
    visited[x][y] = num;
    changeDegree(visited, degree, x, y, 1);
    auto comp = [](const pair<int, pair<int, int>>& p1, pair<int, pair<int, int>>& p2) {
